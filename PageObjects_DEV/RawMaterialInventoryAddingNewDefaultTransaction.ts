...
import { AddingDefaultRawMaterialInventory } from "../TestData_Dev/RawMaterialInventoryDefaultAddingJData";



    export class RawMaterialInventoryAddingNewDefaultTransaction {
        public page : Page;
        ...
        async performAddingNewDefaultTransaction()
        {
          ...
          await this.page.getByLabel('Lot Number: *').fill(AddingDefaultRawMaterialInventory.LOTNUmber);
          console.log(AddingDefaultRawMaterialInventory.LOTNUmber);

          //Selecting Transaction type
          await this.page.getByPlaceholder('Select a transaction code').click();
          await this.page.getByText(AddingDefaultRawMaterialInventory.TransactionCode).click();
          console.log(AddingDefaultRawMaterialInventory.TransactionCode);

          //Selecting Unit of issue 
          await this.page.getByPlaceholder('Select a unit of issue').click();
          await this.page.getByLabel('Items').getByText(AddingDefaultRawMaterialInventory.UnitOfIssueID).click();
          console.log(AddingDefaultRawMaterialInventory.UnitOfIssueID)
          ...
          await this.page.getByLabel('Operator Name:').fill(AddingDefaultRawMaterialInventory.OperatorName);
          await this.page.getByLabel('Keyed By:').click();
          await this.page.getByLabel('Keyed By:').fill(AddingDefaultRawMaterialInventory.KeyedBy);
          await this.page.getByLabel('Note:').click();
          await this.page.getByLabel('Note:').fill(AddingDefaultRawMaterialInventory.Note);
          ...
          await this.page.screenshot({ path: `Screenshots_DEV/5(Default).png`});
