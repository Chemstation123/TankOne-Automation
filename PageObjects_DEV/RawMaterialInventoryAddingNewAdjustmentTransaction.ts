...
import { AddingAdjustmentRawMaterialInventory } from "../TestData_Dev/RawMaterialInventoryAdjustmentAddingJData";
    ...
    export class RawMaterialInventoryAddingNewAdjustmentTransaction {
        public page : Page;
        ...
        async performAddingNewAdjustmentTransaction()
        {
          //const AddingAdjustmentRawMaterialInventory.LOTNUmber = generateRandomAlphaNumeric(5);
          ...
          await this.page.getByLabel('Lot Number: *').fill(AddingAdjustmentRawMaterialInventory.LOTNUmber);
          console.log(AddingAdjustmentRawMaterialInventory.LOTNUmber);

          //Selecting Transaction type
          await this.page.getByPlaceholder('Select a transaction code').click();
          await this.page.getByText(AddingAdjustmentRawMaterialInventory.TransactionCode).click();
          console.log(AddingAdjustmentRawMaterialInventory.TransactionCode);

          //Selecting Unit of issue 
          await this.page.getByPlaceholder('Select a unit of issue').click();
          await this.page.getByLabel('Items').getByText(AddingAdjustmentRawMaterialInventory.UnitOfIssueID).click();
          console.log(AddingAdjustmentRawMaterialInventory.UnitOfIssueID)
          ...
          await this.page.getByLabel('Operator Name:').fill(AddingAdjustmentRawMaterialInventory.OperatorName);
          await this.page.getByLabel('Keyed By:').click();
          await this.page.getByLabel('Keyed By:').fill(AddingAdjustmentRawMaterialInventory.KeyedBy);
          await this.page.getByLabel('Note:').click();
          await this.page.getByLabel('Note:').fill(AddingAdjustmentRawMaterialInventory.Note);
          ...
          await this.page.screenshot({ path: `Screenshots_DEV/1.png`});
